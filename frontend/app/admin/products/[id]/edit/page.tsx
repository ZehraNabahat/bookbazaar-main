...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const { data } = await axios.get(`/api/products/${params.id}?all=true`);
        setFormData({
          name: data.name || '',
          price: data.price || 0,
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold text-navy-900 mb-6">Edit Product</h1>
        ...
        {activeTab === 'basic' && (
          <div className="space-y-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <label htmlFor="pub" className="font-medium text-navy-900">Publish</label>
            </div>
          </div>
        )}
